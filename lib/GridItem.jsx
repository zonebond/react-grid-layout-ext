...
export const ElementBus = {
  ...
  put: function(key, value) {
    const item = this._items_[key];
    if(item) {
      this._items_[key] = { ...item, ...value };
    } else {
      this._items_[key] = value;
    }
  ...
  get: function(key) {
    return this._items_[key];
  },
  notification: function(action) {
    const callbacks = this.handles[action];
    if(!Array.isArray(callbacks)) return;
    callbacks.forEach(c => c(this.toArray()));
  },
  listen: function(action, callback) {
    const callbacks = this.handles[action];
    if(!callbacks) {
      this.handles[action] = [callback];
    } else {
      callbacks.push(callback);
    }
  },
  toArray: function() {
    return Object.keys(this._items_).map(p => ({i: p, ...this._items_[p]}));
  }
};
// HACK ??
...
export default class GridItem extends React.Component<Props, State> {
  ...
  render(): ReactNode {
    ...
    ElementBus.put(key, pos);
    // const fork = this.fork;
    // ElementBus.put(key, fork ? { ...pos, rect: fork.getClientRects() } : pos);
